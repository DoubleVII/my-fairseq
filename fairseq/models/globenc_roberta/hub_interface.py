...
class RobertaHubInterface(nn.Module):
    ...
    def extract_features(
        ...
        if return_all_hiddens:
            ...
            return features, extra["globenc_attributions"]  # just the last layer's features
    ...
    def predict(
        ...
        features, globenc_attributions = self.extract_features(
            tokens.to(device=self.device),
            output_globenc=output_globenc,
            dropout_tokens=dropout_tokens,
        )
        ...
        if return_logits:
            # return logits
            return logits, globenc_attributions
        return F.log_softmax(logits, dim=-1), globenc_attributions
