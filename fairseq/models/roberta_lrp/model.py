...
from fairseq.models.transformer_lrp.lrp_utils import LRPWrapper, LinearWrapper
...
@register_model("roberta_lrp")
class RobertaModel(FairseqEncoderModel, LRPWrapper):
    @classmethod
    def hub_models(cls):
        return {
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert classification_head_name is None
        if classification_head_name is not None:
    ...
    def relprop(self, out_relevance):
        out_relevance = self.encoder.relprop(out_relevance)
...
class RobertaClassificationHead(LRPWrapper):
    ...
    def __init__(
        ...
        super().__init__()
        self.activation_fn = utils.get_activation_fn(activation_fn)
        self.dense = LinearWrapper(
            nn.Linear(input_dim, inner_dim), activation_fn=self.activation_fn
        )
    ...
    def forward(self, features, record=False, **kwargs):
        if record:
            self.record("input", features)
        x = features[:, 0, :]  # take <s> token (equiv. to [CLS])
        x = self.dropout(x)
        x = self.dense(x, record=record)
        # x = self.activation_fn(x) we do it in dense
        x = self.dropout(x)
        x = self.out_proj(x, record=record)
        return x

    def relprop(self, out_relevance):
        out_relevance = self.out_proj.relprop(out_relevance)
        out_relevance = self.dense.relprop(out_relevance)
        input = self.get_record("input")
        in_relevance = torch.zeros_like(input).to(input)
        in_relevance[:, 0, :] = out_relevance
        return in_relevance


class RobertaEncoder(FairseqEncoder, LRPWrapper):
    """RoBERTa encoder."""
        ...
        **unused,
        ...
        Args:
            ...
            return_all_hiddens (bool, optional): also return all of the
                intermediate hidden state>s (default: False).
    ...
    def relprop(self, out_relevance):
        ...
        out_relevance = self.sentence_encoder.relprop(out_relevance)
        return out_relevance


@register_model_architecture("roberta_lrp", "roberta_lrp")
...
@register_model_architecture("roberta_lrp", "roberta_prenorm_lrp")
...
@register_model_architecture("roberta_lrp", "roberta_base_lrp")
def roberta_base_architecture(args):
    base_architecture(args)


@register_model_architecture("roberta_lrp", "roberta_large_lrp")
...
@register_model_architecture("roberta_lrp", "xlm_lrp")
