...
from fairseq.models.transformer_lrp.lrp_utils import LRPWrapper


class RobertaHubInterface(LRPWrapper):
    ...
    def predict(
        ...
        features = self.extract_features(
            tokens.to(device=self.device), record=record, dropout_tokens=dropout_tokens,
        )
        logits = self.model.classification_heads[head](features, record=record)
        if return_logits:
            return logits
    ...
    def relprop(self, out_relevance):
        head = self.get_record("head")
        out_relevance = self.model.classification_heads[head].relprop(out_relevance)
        out_relevance = self.model.relprop(out_relevance)
