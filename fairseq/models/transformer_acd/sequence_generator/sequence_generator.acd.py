...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        self.model.has_incremental = False  # acd do not support incremental
    ...
    def compute_acd(
        ...
        net_input["decomposition_index"] = None
        ...
        for i in range(pred_tokens.size(-1)):
            ...
            for j in range(src_tokens.size(-1)):
                net_input["decomposition_index"] = j
                encoder_outs = self.model.forward_encoder(net_input)
                lprobs, _, compositions_logits = self.model.forward_decoder(
                ...
                # dim2(0): bias compo, dim2(1): gamma compo, dim2(2): beta compo, we select beta compo
                src_scores.append(compositions_logits[0, step_target, 2])

            net_input["decomposition_index"] = None
            tgt_scores = []
            encoder_outs = self.model.forward_encoder(net_input)  # cache encoder out
            for j in range(step_token_prefix.size(-1)):
                lprobs, _, compositions_logits = self.model.forward_decoder(
                    ...
                    [None],
                    self.temperature,
                    decomposition_index=j,
                )
                tgt_scores.append(compositions_logits[0, step_target, 2])

            step_scores = torch.stack(src_scores + tgt_scores, dim=0)
            out_scores_list.append(step_scores)

        all_scores_mat = torch.zeros(
            (len(out_scores_list), out_scores_list[-1].size(0))
        ).to(out_scores_list[-1])

        for i in range(len(out_scores_list)):
            out_scores = out_scores_list[i]
            all_scores_mat[i, : out_scores.size(0)] = out_scores

        net_input["decomposition_index"] = False
        return all_scores_mat
    ...
    def _generate(
        ...
        net_input = sample["net_input"]
        attribution_positional_scores = self.compute_acd(
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if compositions_logits is not None:
                compositions_logits = compositions_logits[:, -1, :, :]
                compositions_logits = compositions_logits.transpose(
                    1, 2
                ).contiguous()  # bsz*beam x dict_size x 3

            if self.models_size == 1:
                return probs, attn, compositions_logits
        ...
        return avg_probs, avg_attn, compositions_logits
