...
@register_model("acd_transformer")
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        if dropout_tokens is not None:
            assert decomposition_index is None
    ...
    def reorder_encoder_out(self, encoder_out: EncoderOut, new_order):
        ...
        new_encoder_compositions = None
        if encoder_compositions is not None:
            new_encoder_compositions = encoder_compositions.index_select(2, new_order)
        ...
        return EncoderOut(
            ...
            encoder_compositions=new_encoder_compositions,
        )
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        if decomposition_index is not None:
            ...
            tgt_len, batch_size, hidden_dim = x.size()

            compositions = torch.zeros((tgt_len, 3, batch_size, hidden_dim)).to(x)
            compositions[:, 1] = x
            compositions[decomposition_index, 1] = 0
            compositions[decomposition_index, 2] = x[decomposition_index, :, :]

            if encoder_compositions is None:
                src_len, batch_size, hidden_dim = encoder_out.encoder_out.size()
                encoder_compositions = torch.zeros(
                    (src_len, 3, batch_size, hidden_dim)
                ).to(x)
                encoder_compositions[:, 1] = encoder_out.encoder_out

        elif encoder_compositions is not None:
            assert incremental_state is None
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, compositions = layer(
                ...
                encoder_compositions=encoder_compositions,
