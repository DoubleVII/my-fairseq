...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        self.model.has_incremental = False  # lrp do not support incremental
    ...
    def compute_relevance(
        ...
        for i in range(pred_tokens.size(-1)):
            ...
            assert logits.size()[:2] == (1, 1)
            relevance = torch.zeros((1, len(step_token_prefix), logits.size(-1))).to(
                logits
            )
            relevance[0, -1, step_target] = logits[0, 0, step_target]
            out_relevance_encoder, out_relevance_decoder = self.model.models[0].relprop(
                relevance
            )
            self.model.models[0].decoder.clear_record()

            out_relevance = torch.cat(
                (out_relevance_encoder, out_relevance_decoder), dim=1
            )
            out_relevance = out_relevance.sum(-1).squeeze(0)
            out_relevance_list.append(out_relevance)

        all_relevance_mat = torch.zeros(
            (len(out_relevance_list), out_relevance_list[-1].size(0))
        ).to(out_relevance_list[-1])

        self.model.models[0].clear_record()
        for i in range(len(out_relevance_list)):
            out_relevance = out_relevance_list[i]
            all_relevance_mat[i, : out_relevance.size(0)] = out_relevance

        net_input["record"] = False
        return all_relevance_mat
    ...
    def _generate(
        ...
        net_input = sample["net_input"]
        attribution_positional_scores = self.compute_relevance(
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        return avg_probs, avg_attn, logits
