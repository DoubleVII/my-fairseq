...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        value_composition: Optional[pydec.Composition] = None,
        ...
        # pydec: initialize error_tensor_mask for composition
        if key_padding_mask is not None:
            error_tensor_mask = key_padding_mask.T.unsqueeze(-1)
        else:
        ...
        if self.self_attention:
            ...
            k = self.k_proj(query)
            v = self.v_proj(query)
            if value_composition is not None:
                value_composition = pydec.nn.functional.linear(
                    value_composition, self.v_proj.weight, self.v_proj.bias,
                )
                pydec.check_error(
                    value_composition, ref=v, error_tensor_mask=error_tensor_mask,
                )
        elif self.encoder_decoder_attention:
            ...
            # pydec: we never consider this situation
        ...
        if saved_state is not None:
            ...
            if "prev_value_composition" in saved_state:
                _prev_value_composition = saved_state["prev_value_composition"]
                assert _prev_value_composition is not None
                assert kv_bsz == _prev_value_composition.size(0)
                prev_value_composition = _prev_value_composition.view(
                    kv_bsz * self.num_heads, -1, self.head_dim
                )
                if static_kv:
                    value_composition = prev_value_composition
                else:
                    assert value_composition is not None
                    if value_composition.numc() != prev_value_composition.numc():
                        padding_numc = (
                            value_composition.numc() - prev_value_composition.numc()
                        )
                        padding_composition = pydec.Composition(
                            prev_value_composition.size(), component_num=padding_numc
                        ).to(prev_value_composition)
                        prev_value_composition = pydec.c_cat(
                            [prev_value_composition, padding_composition]
                        )
                    value_composition = pydec.cat(
                        [prev_value_composition, value_composition], dim=1
                    )
            ...
            if value_composition is not None:
                saved_state["prev_value_composition"] = value_composition.view(
                    kv_bsz, self.num_heads, -1, self.head_dim
                )
        ...
        if value_composition is not None:
            # pydec: only support Composition @ Tensor
            attn_value_composition: pydec.Composition = value_composition.transpose(
                -1, -2
            ) @ attn_probs.transpose(-1, -2)
            attn_value_composition = attn_value_composition.transpose(-1, -2)
        ...
        if self.onnx_trace and attn.size(1) == 1:
            ...
            if value_composition is not None:
                attn_value_composition = (
                    attn_value_composition.transpose(0, 1)
                    .contiguous()
        ...
        if value_composition is not None:
            attn_value_composition = pydec.nn.functional.linear(
                attn_value_composition, self.out_proj.weight, self.out_proj.bias,
            )
            pydec.check_error(attn_value_composition, ref=attn)
        ...
        if value_composition is not None:
            return attn, attn_weights, attn_value_composition
        else:
