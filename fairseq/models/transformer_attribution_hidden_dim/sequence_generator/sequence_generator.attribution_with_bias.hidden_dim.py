...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        decomposition_scores = torch.zeros(
            bsz * beam_size, max_len + 1, encoder_outs[0].encoder_compositions.size(1)
        )
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            composition_size = compositions_logits.size(-1)
            decomposition_cand_scores = compositions_logits.view(
                bsz, -1, composition_size
            ).gather(
                dim=1,
                index=beam_cand_indices.unsqueeze(-1).expand(-1, -1, composition_size),
            )
            ...
            decomposition_scores.view(bsz, beam_size, -1, composition_size)[
                :, :, step
            ] = torch.gather(
                decomposition_cand_scores,
                dim=1,
                index=active_hypos.unsqueeze(-1).expand(-1, -1, composition_size),
            )
        ...
        self.dim_scores_save(
            decomposition_positional_scores,
            finalized[0][0]["tokens"],
            3003,
            "testset_dim_scores_data.pt",
        )
        ...
        #     file_name="nh_test_decoder_norms_logits.pt",
        # )
        ...
        #         sample_num=sample_num,
        #         file_name="nh_test_encoder_norms.pt",
        #     )
    ...
    def dim_scores_save(
        ...
        tokens: Tensor,
        sample_num: int,
        file_name: str = "dim_scores_data.pt",
    ):
        if "dim_scores" not in dir(self):
            self.dim_scores = []
            self.dim_tokens = []

        words: List[str] = self.tgt_dict.string(tokens).split()
        assert len(words) == len(tokens) - 1
        words.append("<eos>")

        for score, word in zip(decomposition_positional_scores, words):
            self.dim_scores.append(score.float().cpu())
            self.dim_tokens.append(word)

        if self.count == sample_num:
            data_dict = {}
            data_dict["dim_scores"] = self.dim_scores
            data_dict["dim_tokens"] = self.dim_tokens
    ...
    def dim_distribution(
        ...
        decomposition_positional_scores: Tensor,
        sample_num: int,
        file_name: str = "dim_distribution_data.pt",
    ):
        if "top20_count_distribution" not in dir(self):
            self.top20_count_distribution = torch.zeros(
                (decomposition_positional_scores.size(-1),), dtype=torch.long
            ).cpu()
        if "positive_logit_distribution" not in dir(self):
            self.positive_logit_distribution = torch.zeros(
                (decomposition_positional_scores.size(-1),),
            ).cpu()
        if "logit_distribution" not in dir(self):
            self.logit_distribution = torch.zeros(
                (decomposition_positional_scores.size(-1),),
            ).cpu()
        topk_index = decomposition_positional_scores.topk(k=20, dim=-1)[1].cpu()
        for index in topk_index:
            self.top20_count_distribution[index] += 1
        self.logit_distribution += decomposition_positional_scores.sum(dim=0).cpu()
        positive_logit_scores = decomposition_positional_scores
        positive_logit_scores[positive_logit_scores < 0] = 0.0
        self.positive_logit_distribution += positive_logit_scores.sum(dim=0).cpu()
        if self.count == sample_num:
            data_dict = {}
            data_dict["top20_count_distribution"] = self.top20_count_distribution
            data_dict["logit_distribution"] = self.logit_distribution
            data_dict["positive_logit_distribution"] = self.positive_logit_distribution
            torch.save(data_dict, file_name)
