...
@register_model("attribution_hidden_dim_transformer")
...
class TransformerEncoder(BaseTransformerEncoder):
    ...
    def forward(
        ...
        if return_compositions:
            ...
            compositions = torch.zeros((src_len, hidden_dim, batch_size, hidden_dim)).to(
                x
            )
            index = torch.arange(0, hidden_dim).to(src_tokens)
            compositions[:, index, :, index] = x.permute(2, 0, 1)
...
class TransformerDecoder(BaseTransformerDecoder):
    ...
    def extract_features_scriptable(
        ...
        if return_compositions:
            ...
            compositions = torch.zeros((1, hidden_dim, batch_size, hidden_dim,)).to(x)
            index = torch.arange(0, hidden_dim).to(prev_output_tokens)
            compositions[:, index, :, index] = x.permute(2, 0, 1)

            compositions = torch.cat(
                [torch.zeros((1, 1, batch_size, hidden_dim)).to(x), compositions], dim=1,
            )
        else:
            compositions = None
