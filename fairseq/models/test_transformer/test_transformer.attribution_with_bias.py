...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, return_all_hiddens: bool = False):
        ...
        compositions = torch.zeros((src_len, src_len, batch_size, hidden_dim)).to(x)
        index = torch.arange(0,src_len).to(src_tokens)
        compositions[index,index,:,:] = x

        compositions = torch.cat([torch.zeros((src_len, 1, batch_size, hidden_dim)).to(x), compositions],dim=1)
        ...
        for layer in self.layers:
            x,compositions = layer(x, encoder_padding_mask, compositions=compositions)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        compositions = torch.zeros((1, encoder_out.encoder_out.size(0)+prev_token_len+1, batch_size, hidden_dim)).to(x)
        compositions[:,-1,:,:] = x
        ...
        isolate_hidden = (x, torch.zeros_like(x).to(x))
        inner_isolate_hiddens:List[Optional[Tensor]] = [isolate_hidden]
        ...
        for idx, layer in enumerate(self.layers):
            ...
            inner_isolate_hiddens.append(isolate_hidden)
        ...
        return x, {"attn": [attn], "inner_states": inner_states, "compositions":compositions}
