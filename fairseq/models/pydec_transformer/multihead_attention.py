...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        def nan_check(input):
            return torch.any(torch.isnan(input))

        def print_max(prefix, input):
            if isinstance(input, pydec.Composition):
                print(prefix, input.residual.abs().max())
            else:
                print(prefix, input.abs().max())

        # print_max("    max before sofmax: ", attn_weights)

        attn_weights_float = utils.softmax(
            attn_weights, dim=-1, onnx_trace=self.onnx_trace
        )

        # print_max("    max after sofmax: ", attn_weights_float)
        ...
        if self.encoder_decoder_attention and bsz != kv_bsz:
            ...
            assert not isinstance(attn_probs, pydec.Composition)
            if isinstance(v, pydec.Composition):
                if (
                    isinstance(attn_probs, pydec.Composition)
                    and attn_probs.numc() != v.numc()
                ):
                    padding_num = attn_probs.numc() - v.numc()
                    c_padding = pydec.zeros(v.size(), c_num=padding_num).to(v)
                    v = pydec.c_cat([v, c_padding], sum_residual=True)
                if isinstance(attn_probs, pydec.Composition):
                    # attn0 = torch.bmm(attn_probs.c_sum(), v)
                    # attn1 = torch.bmm(attn_probs, v.c_sum())
                    # Lambda = 0.1
                    # attn = Lambda * attn0 + (1 - Lambda) * attn1
        ...
        # print_max("before attn: ", query)
        # print_max("after attn: ", attn)
