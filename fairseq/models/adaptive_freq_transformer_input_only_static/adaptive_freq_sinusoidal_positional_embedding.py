...
class AdaptiveFreqSinusoidalPositionalEmbedding(nn.Module):
    """This module produces adaptive frequency sinusoidal positional embeddings.

    Padding symbols are ignored.
    """

    def __init__(self, embedding_dim, padding_idx, phase=False, freq_base=200.0):
        super().__init__()
        ...
        self.freq_weights = torch.linspace(
            0, 1, half_dim, dtype=torch.float, requires_grad=False
        )

        self.phase_weights = None
        if phase:
            self.phase_weights = nn.Parameter(torch.zeros((half_dim), dtype=torch.float))
