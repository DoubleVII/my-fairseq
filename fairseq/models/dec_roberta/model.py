...
# from fairseq.models.transformer.attribution_composition_utils import (
#     check_error,
#     forward_linear,
#     forward_relu,
...
class RobertaModel(FairseqEncoderModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, extra, composition = self.encoder(
            src_tokens,
            features_only,
            return_all_hiddens,
        ...
        if classification_head_name is not None:
            x, composition = self.classification_heads[classification_head_name](
                x, composition
            )
        return x, extra, composition
...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, composition: Composition = None, **kwargs):
        ...
        composition = pydec.nn.functional.linear(
            composition, self.dense.weight, self.dense.bias
        )
        # do relu
        composition = pydec.nn.functional.relu(composition, ref=ref)
        composition = pydec.nn.functional.linear(
            composition, self.out_proj.weight, self.out_proj.bias
        )
...
class RobertaEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        **unused,
        ...
        """
        x, extra, composition = self.extract_features(
            src_tokens,
    ...
    def extract_features(
        ...
        **kwargs,
        ...
        # T x B x C -> B x T x C
        composition: Composition = encoder_out["encoder_composition"]
        if composition is not None:
            composition = composition.transpose(0, 1)
        # composition_logits = self.output_layer(composition)
        features = encoder_out["encoder_out"][0].transpose(0, 1)
        inner_states = encoder_out["encoder_states"] if return_all_hiddens else None
        return features, {"inner_states": inner_states}, composition
