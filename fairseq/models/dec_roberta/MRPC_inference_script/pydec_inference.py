...
data_len = 0
with open(f"/home/yangs/data/glue_data/MRPC/{test_set}.tsv") as fin:
    fin.readline()
    for line in fin:
        data_len += 1
...
roberta = RobertaModel.from_pretrained(
    ...
    data_name_or_path="/home/yangs/data/data-bin/MRPC-bin",
)
        ...
        for index, line in enumerate(fin):
            ...
            if nsamples % 50 == 0:
                ...
                for k, v in drop_values.items():
                    logger.info(
                        f"AOPC for k={k}: {torch.mean(torch.stack(drop_values[k]))}"
                    )
