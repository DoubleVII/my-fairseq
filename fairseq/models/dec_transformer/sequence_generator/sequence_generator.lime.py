...
explainer = LimeTextExplainer(bow=False, mask_string="<MASK>")
...
class SequenceGenerator(nn.Module):
    ...
    def lime_explain(
        ...
        def pred_fn(
            ...
            for i in range(len(input)):
                mask_tensor.append(
                    list(map(lambda t: t == mask_token, input[i].split()))
                )
            mask_tensor = torch.tensor(mask_tensor, dtype=torch.bool).to(
                device=src_tokens.device
            )

            batched_src_tokens = src_tokens.repeat(len(input), 1)

            batched_src_tokens[mask_tensor[:, :src_len]] = padding_idx

            batched_prefix_tokens = prefix_tokens[None, :tgt_len].repeat(len(input), 1)
            batched_prefix_tokens[mask_tensor[:, src_len:]] = padding_idx

            new_net_input = {}
            new_net_input["src_tokens"] = batched_src_tokens
            new_net_input["src_lengths"] = net_input["src_lengths"].repeat(len(input))
            new_net_input["prev_output_tokens"] = None

            encoder_outs = self.model.forward_encoder(new_net_input)
            lprobs, avg_attn_scores = self.model.forward_decoder(
                batched_prefix_tokens,
                encoder_outs,
            ...
            out = torch.exp(lprobs).cpu().numpy()
            return out
        ...
        tgt_len = len(prefix_tokens)
        scores = -torch.ones((tgt_len, src_len + tgt_len), dtype=torch.float).to(
            device=src_tokens.device
        )

        for i in range(tgt_len):
            token_len = src_len + i + 1
            pseudo_token = " ".join(map(str, range(token_len)))

            exp = explainer.explain_instance(
                pseudo_token,
                functools.partial(
                    pred_fn, src_len=src_len, tgt_len=i + 1, net_input=net_input
                ),
                labels=(pred_tokens[i].item(),),
                num_features=num_features,
                num_samples=256,
            )
            assert pred_tokens[i].item() in exp.as_map()

            for pos_id, lime_score in exp.as_map()[pred_tokens[i].item()]:
                scores[i, pos_id] = lime_score
        return scores
    ...
    def _generate(
        ...
        attribution_positional_scores = self.lime_explain(
            net_input,
