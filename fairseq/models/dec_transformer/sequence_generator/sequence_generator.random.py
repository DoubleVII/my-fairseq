...
class SequenceGenerator(nn.Module):
    ...
    # def random_drop(self, net_input: Dict[str, torch.Tensor],
    #     pred_tokens,
    #     bos_token: Optional[int] = None,):
    #     assert tokens.dim() == 1
    #     # if probs is None:
    ...
    def compute_scores(
        ...
        pred_len = positional_probs.size(0)
        src_tokens = net_input["src_tokens"]
        ...
        for drop_num in ks:
            ...
            for i in range(pred_len):
                ...
                top_indices = list(range(src_len + i + 1))
                random.shuffle(top_indices)
                top_indices = torch.LongTensor(top_indices[:tokens_to_keep]).to(
                    src_tokens
                )
