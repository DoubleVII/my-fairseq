...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        x, _, composition = self.self_attn(
            query=x,
            key=x,
            value=x,
        ...
        if composition is not None:
            composition = self.residual_connection(composition, composition_residual)
        ...
        if composition is not None:
            ...
            composition = pydec.nn.functional.relu(composition)
            composition = pydec.nn.functional.linear(
        ...
        if not self.normalize_before:
            ...
            if composition is not None:
                pydec.check_error(
                    composition, ref=x, error_tensor_mask=error_tensor_mask
                )
...
class TransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        x, attn, attn_value_composition = self.self_attn(
            query=x,
            key=y,
            value=y,
        ...
        if composition is not None:
            composition = self.residual_connection(
                attn_value_composition, composition_residual
            )
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            x, attn, attn_value_composition = self.encoder_attn(
                query=x,
                key=encoder_out,
                value=encoder_out,
                key_padding_mask=encoder_padding_mask,
                ...
                value_composition=encoder_composition,
            )

            if composition is not None:
                # pydec: padding component number of encoder_composition to
                # source-size token number + target-size previous token number
                padding_numc = composition.numc() - attn_value_composition.numc()
                padding_composition = pydec.zeros(
                    attn_value_composition.size(), c_num=padding_numc
                ).to(x)
                attn_value_composition = pydec.c_cat(
                    [attn_value_composition, padding_composition], sum_residual=False
                )
