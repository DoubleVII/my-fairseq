...
class SequenceGenerator(nn.Module):
    ...
    def hack_input(self, net_input):
        ...
        new_src_tokens = self.tgt_dict.encode_line(src_string, add_if_not_exist=False).to(
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            with torch.autograd.profiler.record_function(
                ...
                if step == isolate_step:
                    isolate_list = list(range(1, 13))
                    isolate_list.remove(5)
                else:
                    isolate_list = None
            ...
            if self.mode == "token":
                ...
                composition_scores.view(bsz, beam_size, -1)[:, :, :, step] = pydec.gather(
                    composition_cand_scores,
                    dim=1,
                    index=active_hypos,
                )
        ...
        print(composition_positional_scores._composition_tensor[:, -3].round(decimals=1))
        import pdb
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                ...
                if hasattr(model, "decoder"):
                    decoder_out = model.decoder.forward(
                        tokens, encoder_out=encoder_out, decompose=decompose, **kwargs
                    )
                else:
                    decoder_out = model.forward(tokens, decompose=decompose, **kwargs)
