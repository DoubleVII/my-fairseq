...
def isolate_relu(
    ...
    *,
    ...
    if isolate_list is not None:
        assert 0 not in isolate_list
        keep_list = []
        for i in range(len(input)):
            if i not in isolate_list:
                keep_list.append(i)
        keep_list = torch.LongTensor(
            keep_list,
        ).to(device=input._composition_tensor.device)
        keep_composition = input.c_index_select(index=keep_list)
        keep_zero_mask = keep_composition.c_sum() < 0
        apply_list = torch.LongTensor(
            apply_list,
        ).to(device=input._composition_tensor.device)
        applied_composition = input[apply_list].masked_fill(keep_zero_mask, 0.0)
    out = input.masked_fill(zero_mask, 0.0)

    if isolate_list is not None:
        out[apply_list] = applied_composition
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        if composition is not None:
            ...
            attn_composition[1 + 2 * layer_id] = attn_x
            composition = self.residual_connection(attn_composition, composition_residual)
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            if composition is not None:
                ...
                attn_composition[2 + 2 * layer_id] = attn_x
        ...
        if composition is not None:
            ...
            if isolate_list is not None:
                composition = isolate_relu(composition, isolate_list=isolate_list)
            else:
                composition = pydec.nn.functional.relu(composition)
            if self.ffn_layernorm is not None:
                composition = pydec.nn.functional.layer_norm_1d(
