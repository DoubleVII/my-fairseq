...
class SequenceGenerator(nn.Module):
    ...
    def leave_one_out(
        ...
        for i in range(tgt_len):
            for j in range(src_len):
                dropout_tokens = torch.LongTensor([j]).to(src_tokens)
                net_input["dropout_tokens"] = dropout_tokens
                ...
                scores[i, j] = positional_probs[i] - torch.exp(
                    lprobs[0, pred_tokens[i]]
                )

            net_input["dropout_tokens"] = None
            encoder_outs = self.model.forward_encoder(net_input)
            for j in range(i + 1):
                dropout_tokens = torch.LongTensor([j]).to(src_tokens)
                lprobs, avg_attn_scores = self.model.forward_decoder(
                ...
                scores[i, src_len + j] = positional_probs[i] - torch.exp(
                    lprobs[0, pred_tokens[i]]
                )
    ...
    def _generate(
        ...
        attribution_positional_scores = self.leave_one_out(
            net_input,
