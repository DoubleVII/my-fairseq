...
@overload
def _shift_dims(dims: _size) -> Size:
    ...


@overload
def _shift_dims(*dims: _int) -> Size:
    ...


def _shift_dims(*dims) -> Size:
    if len(dims) == 1:
        dims = dims[0]
    out = list(dims)
...
class Composition:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if isinstance(args[0], (Size, list, Tuple)):
            if len(args) != 2:
    ...
    def size(self, dim: Optional[_int] = None) -> Union[Size, _int]:
        if dim is None:
    ...
    def c_size(self, dim: Optional[_int] = None) -> Union[Size, _int]:
        if dim is None:
    ...
    def __add__(self, other) -> Composition:
        if isinstance(other, Composition):
            ...
            if decomposition_func is not None:
                ...
                _from_replce(self._composition_tensor, self._residual_tensor + other)
            return self
        else:
            raise unsupported_operand_error("+", type(self), type(other))
    ...
    def sum(
        ...
        if dim is None:
            ...
            if isinstance(dim, _int):
                dim = (dim,)
            dims = _shift_dims(dim)
            out_composition_tensor = self._composition_tensor.sum(
                dim=dims, keepdim=keepdim, dtype=dtype
            )
        return _from_replce(out_composition_tensor, out_residual_tensor)
