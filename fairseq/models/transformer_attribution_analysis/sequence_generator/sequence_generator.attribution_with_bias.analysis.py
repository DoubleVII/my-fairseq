...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        self.decoder_norm_logit_analysis(
            ...
            file_name="cand_nh_test_decoder_norms_logits.pt",
        )

        self.decoder_attn_analysis(
            analysis_info_list,
            sample_num=sample_num,
            file_name="cand_nh_test_decoder_attn.pt",
        )
        ...
        if encoder_compositions_states is not None:
            self.encoder_norm_analysis(
                ...
                sample_num=sample_num,
                file_name="cand_nh_test_encoder_norms.pt",
            )
        if encoder_attn_states is not None:
            self.encoder_attn_analysis(
                encoder_attn_states,
                sample_num=sample_num,
                file_name="cand_nh_test_encoder_attn.pt",
            )
