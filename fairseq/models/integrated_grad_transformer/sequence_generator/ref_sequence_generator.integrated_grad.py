...
class SequenceGenerator(nn.Module):
    ...
    def compute_grad(self, net_input, tokens):
        import numpy as np

        tokens = torch.cat([torch.LongTensor([2,]).to(tokens), tokens], dim=0)[None, :]
        src_tokens = net_input["src_tokens"][0]
        ...
        with torch.enable_grad():
            ...
            for i in range(tokens.size(-1) - 1):
                ...
                for j in range(src_tokens.size(-1)):
                    sum_grads = None
                    orig_embed = (
                        embed_tokens(net_input["src_tokens"]).clone().detach()[0, j, :]
                    )
                    for alpha in np.linspace(0, 1.0, num=num_steps, endpoint=False):
                        embed = embed_tokens(net_input["src_tokens"]).clone().detach()
                        embed[0, j, :] = embed[0, j, :] * alpha
                        embed.requires_grad_(True)
                        self.model.models.zero_grad()
                        net_input["input_embeddings"] = embed
                        encoder_outs = self.model.forward_encoder(net_input)
                        lprobs, _ = self.model.forward_decoder(
                            token_prefix, encoder_outs, [None], self.temperature,
                        )
                        loss = -lprobs[0, target]
                        loss.backward()
                        if sum_grads is None:
                            sum_grads = embed.grad[0, j, :].clone().detach()
                        else:
                            sum_grads = sum_grads + embed.grad[0, j, :].clone().detach()
                    sum_grads = sum_grads / num_steps
                    sum_grads *= orig_embed
                    src_grads.append(sum_grads)

                tgt_grads = []
                for j in range(token_prefix.size(-1)):
                    sum_grads = None
                    orig_embed = embed_tokens(token_prefix).clone().detach()[0, j, :]
                    for alpha in np.linspace(0, 1.0, num=num_steps, endpoint=False):
                        embed = embed_tokens(token_prefix).clone().detach()
                        embed[0, j, :] = embed[0, j, :] * alpha
                        embed.requires_grad_(True)
                        self.model.models.zero_grad()
                        # net_input["input_embeddings"] = embed
                ...
                # src_grads = torch.stack(src_grads, dim=0)
                # src_grads = torch.stack(src_grads, dim=0)
                all_grads = torch.stack(src_grads + tgt_grads, dim=0)
                all_grads = all_grads.sum(dim=-1)
                all_grads = torch.abs(all_grads) / torch.norm(all_grads, dim=0, p=1)
                all_scores.append(all_grads)

            all_scores_mat = torch.zeros((len(all_scores), all_scores[-1].size(0))).to(
                all_scores[-1]
            )
            for i in range(len(all_scores)):
                score = all_scores[i]
                all_scores_mat[i, : score.size(0)] = score
            import pdb

            pdb.set_trace()
            print(all_scores_mat)
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = self.model.forward_decoder(
                tokens[:, : step + 1], encoder_outs, incremental_states, self.temperature,
            )
        ...
        for sent in range(len(finalized)):
            ...
            self.compute_grad(net_input, finalized[sent][0]["tokens"])
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                ...
                decoder_out = model.decoder.forward(
                    tokens, encoder_out=encoder_out, input_embeddings=input_embeddings
                )
