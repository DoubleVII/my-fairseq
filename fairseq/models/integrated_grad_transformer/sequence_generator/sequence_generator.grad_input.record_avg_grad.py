    ...
    def compute_grad(
        ...
        with torch.enable_grad():
            ...
            for i in range(pred_tokens.size(-1)):
                ...
                tgt_grads = []
                encoder_embed = (
                    embed_tokens(src_tokens[None]).clone().detach()
                )  # 1 x T x C
                encoder_embed.requires_grad_(True)
                self.model.models.zero_grad()
                net_input["input_embeddings"] = encoder_embed
                encoder_outs = self.model.forward_encoder(net_input)

                decoder_embed = (
                    embed_tokens(step_token_prefix[None]).clone().detach()
                )  # 1 x T x C
                decoder_embed.requires_grad_(True)
                lprobs, _ = self.model.forward_decoder(
                    step_token_prefix[None],
                    encoder_outs,
                    [None],
                    self.temperature,
                    input_embeddings=decoder_embed,
                )

                loss = -torch.sum(lprobs[:, step_target])
                loss.backward()
                for j in range(src_tokens.size(-1)):
                    sum_grads = encoder_embed.grad[0, j, :].clone().detach()
                    sum_grads *= encoder_embed.grad[0, j, :].detach()
                    src_grads.append(sum_grads)
                    sample_no_int_grad.append(
                        encoder_embed.grad[0, j, :].clone().detach()
                    )

                for j in range(step_token_prefix.size(-1)):
                    sum_grads = decoder_embed.grad[0, j, :].clone().detach()
                    sum_grads *= decoder_embed.grad[0, j, :].detach()
                    tgt_grads.append(sum_grads)
                    sample_no_int_grad.append(
                        decoder_embed.grad[0, j, :].clone().detach()
                    )
                ...
                no_int_grad.append(
                    torch.stack(sample_no_int_grad, dim=0).abs().mean(dim=0)
                )
        ...
        for drop_num in ks:
            ...
            for i in range(pred_len):
                ...
                _, top_indices = torch.topk(
                    attribution_positional_scores[i, : src_len + i + 1],
                    k=tokens_to_keep,
                )
