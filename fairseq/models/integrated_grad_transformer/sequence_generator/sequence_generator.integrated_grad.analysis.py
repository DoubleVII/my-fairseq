...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        self.decoder_ig_scores_analysis(
            attribution_positional_scores,
            finalized[0][0]["tokens"],
            sample_num=sample_num,
            file_name="nh_test_decoder_ig_scores.pt",
        )
    ...
    def decoder_ig_scores_analysis(
        self,
        scores,
        pred_tokens,
        sample_num=10,
        file_name="decoder_ig_scores_data_rec.pt",
    ):
        if "decoder_ig_score_list" not in dir(self):
            self.decoder_ig_score_list = []
        score_layers = [scores]
        self.decoder_ig_score_list.append(score_layers)
        if self.count == sample_num:
            torch.save(
                self.decoder_ig_score_list, file_name,
            )
