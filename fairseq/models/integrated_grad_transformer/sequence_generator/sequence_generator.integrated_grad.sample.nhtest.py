...
class SequenceGenerator(nn.Module):
    ...
    def compute_grad(
        ...
        embed_tokens = self.model.models[0].encoder.embed_tokens
        net_input["input_embeddings"] = None
        num_steps = 20
        with torch.enable_grad():
            self.model.models.requires_grad_(True)
            all_grads = []
            for i in range(pred_tokens.size(-1)):
                step_token_prefix = prefix_tokens[: i + 1]
                step_target = pred_tokens[i]
                # test src tokens
                src_grads = []
                for j in range(src_tokens.size(-1)):
                    sum_grads = None
                    orig_embed = embed_tokens(src_tokens).clone().detach()[j, :]

                    alphas = torch.from_numpy(
                        np.linspace(0, 1.0, num=num_steps, endpoint=False)
                    ).to(orig_embed)
                    embed = embed_tokens(src_tokens[None]).clone().detach()  # 1 x T x C
                    embed = embed.repeat((len(alphas), 1, 1))  # steps x T x C
                    embed[:, j, :] *= alphas[:, None]
                    embed.requires_grad_(True)
                    self.model.models.zero_grad()
                    net_input["input_embeddings"] = embed
                    encoder_outs = self.model.forward_encoder(net_input)
                    lprobs, _ = self.model.forward_decoder(
                        step_token_prefix[None].expand((num_steps, -1)),
                        encoder_outs,
                        [None],
                        self.temperature,
                    )

                    loss = -torch.sum(lprobs[:, step_target])
                    loss.backward()
                    sum_grads = embed.grad[:, j, :].clone().detach().sum(dim=0)
                    sum_grads = sum_grads / num_steps
                    sum_grads *= orig_embed
                    src_grads.append(sum_grads)

                tgt_grads = []
                net_input["input_embeddings"] = None
                net_input["src_tokens"] = net_input["src_tokens"].expand(
                    (num_steps, -1)
                )
                net_input["src_lengths"] = net_input["src_lengths"].expand(
                    (num_steps, -1)
                )
                for j in range(step_token_prefix.size(-1)):
                    sum_grads = None
                    orig_embed = embed_tokens(step_token_prefix).clone().detach()[j, :]

                    alphas = torch.from_numpy(
                        np.linspace(0, 1.0, num=num_steps, endpoint=False)
                    ).to(orig_embed)
                    embed = (
                        embed_tokens(step_token_prefix[None]).clone().detach()
                    )  # 1 x T x C
                    embed = embed.repeat((len(alphas), 1, 1))  # steps x T x C
                    embed[:, j, :] *= alphas[:, None]
                    embed.requires_grad_(True)
                    ...
                    sum_grads = sum_grads / num_steps
                    sum_grads *= orig_embed
                    tgt_grads.append(sum_grads)

                step_grads = torch.stack(src_grads + tgt_grads, dim=0)
                step_grads = step_grads.sum(dim=-1)
                step_grads = torch.abs(step_grads) / torch.norm(step_grads, dim=0, p=1)
                all_grads.append(step_grads)
            all_grads_mat = torch.zeros((len(all_grads), all_grads[-1].size(0))).to(
                all_grads[-1]
            )
            for i in range(len(all_grads)):
                grad = all_grads[i]
                all_grads_mat[i, : grad.size(0)] = grad

            # recover net_input
            net_input["src_tokens"] = src_tokens[None]
            net_input["src_lengths"] = src_lengths
        return all_grads_mat
    ...
    def _generate(
        ...
        net_input = sample["net_input"]
        attribution_positional_scores = self.compute_grad(
            net_input, finalized[0][0]["tokens"]
        )

        conposition_tensor = attribution_positional_scores.T
    ...
    def compute_scores(
        self,
        attribution_positional_scores: Tensor,
        net_input,
        ...
        # attribution_positional_scores = attribution_positional_scores[:, 1:]
        ...
        pred_len = attribution_positional_scores.size(0)
        src_tokens = net_input["src_tokens"]
        ...
        assert src_len + pred_len == attribution_positional_scores.size(1)
        ...
        for i in range(pred_len):
            ...
            _, top_indices = torch.topk(
                attribution_positional_scores[i, : src_len + i + 1], k=tokens_to_keep
            )
