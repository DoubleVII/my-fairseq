...
EncoderOut = NamedTuple(
    ...
    [
        ...
        ("attn_states", Optional[List[Tensor]]),  # List[H x B x Q x K]
    ],
)
...
@register_model("integrated_grad_transformer")
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        if dropout_tokens is not None:
            ...
            encoder_embedding = self.embed_scale * input_embeddings
            x = encoder_embedding + self.embed_positions(src_tokens)
        else:
        ...
        attn_states = [] if need_attn else None

        # encoder layers
        for layer in self.layers:
            x, attn_weights = layer(x, encoder_padding_mask, need_attn=need_attn)
    ...
    def reorder_encoder_out(self, encoder_out: EncoderOut, new_order):
        ...
        attn_states = encoder_out.attn_states
        # if attn_states is not None:
        #     for idx, state in enumerate(attn_states):
        #         attn_states[idx] = state.index_select(1, new_order)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        if dropout_tokens is not None:
            ...
            x = self.embed_scale * input_embeddings
        else:
            # embed tokens and positions
