...
def check_norm(x, s1=None, s2=None, fc=None, fc_weight=None):
    max_norm = torch.max(torch.abs(x))
    if max_norm > 100:
        print("-----------------------------------------------")
        print("current tensor:")
        # print(x)
        # print("max:", max_norm.item())
        # print("4000num:", torch.sum(torch.abs(x) > 4000).item())
        if s1 is not None:
            max_norm = torch.max(torch.abs(s1))
            print("sublayer1:")
            # print(s1)
            print("max:", max_norm.item())
        if s2 is not None:
            max_norm = torch.max(torch.abs(s2))
            print("sublayer2")
            # print(s2)
            print("max:", max_norm.item())
        if fc is not None:
            max_norm = torch.max(torch.abs(fc))
            print("fc")
            # print(fc)
            print("max:", max_norm.item())
        if fc_weight is not None:
            max_norm1 = torch.max(torch.abs(fc_weight[0]))
            max_norm2 = torch.max(torch.abs(fc_weight[1]))
            # print(fc_weight)
            print("weight max:", max_norm1.item())
            print("bias max:", max_norm2.item())
            assert False
...
class DLCLTransformerModel(FairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--encoder-history-type", help="encoder layer history type")
        parser.add_argument("--decoder-history-type", help="decoder layer history type")
        parser.add_argument(
            ...
            for old, new in layer_norm_map.items():
                for m in ("weight", "bias"):
                    ...
                    if k in state_dict:
                        ...
                        del state_dict[k]
        if (
            utils.item(state_dict.get("{}.version".format(name), torch.Tensor([1]))[0])
            < 2
        ):
            # earlier checkpoints did not normalize after the stack of layers
            self.layer_norm = None
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        x = residual + x
        # x_sublayer1 = x
        # check_norm(x.detach(), x_sublayer1.detach())
...
def base_architecture(args):
    ...
    args.max_relative_length = getattr(
        args, "max_relative_length", args.max_relative_length
    )
...
@register_model_architecture(
    "dlcl_transformer", "dense_relative_transformer_t2t_wmt_en_de"
)
