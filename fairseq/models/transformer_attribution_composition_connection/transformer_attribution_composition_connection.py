...
from .transformer_layer_composition_connection import (
...
EncoderOut = NamedTuple(
    ...
    [
        ...
        ("connection_states", Optional[List[Tensor]]),  # List[T x T x B x C]
    ],
)


@register_model("attribution_composition_connection_transformer")
    ...
    def forward(
        ...
        if return_compositions:
            ...
            compositions = torch.cat(
                [torch.zeros((src_len, 1, batch_size, hidden_dim)).to(x), compositions],
                dim=1,
            )
        else:
            compositions = None
        ...
        compositions_states: List[Optional[Tensor]] = []
        connection_states: List[Optional[Tensor]] = []
        ...
        for layer in self.layers:
            ...
            x, compositions, attn_weights, connection_compositions = layer(
                x,
            ...
            if compositions is not None:
                compositions_states.append(compositions)
            if connection_compositions is not None:
                connection_compositions = torch.norm(connection_compositions, dim=-1, p=2)
...
class TransformerDecoder(BaseTransformerDecoder):
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            #     compositions=compositions,
            # )
            x, layer_attn, compositions, connection_compositions = layer(
                x,
                encoder_out.encoder_out if encoder_out is not None else None,
                encoder_out.encoder_padding_mask if encoder_out is not None else None,
                incremental_state,
