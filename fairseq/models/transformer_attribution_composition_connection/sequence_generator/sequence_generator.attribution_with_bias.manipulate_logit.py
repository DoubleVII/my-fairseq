...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            def get_positions(filter_token: str, src_tokens=None):
                if src_tokens is None:
                    src_tokens = self.src_strs[self.count].split()
                pos = []
                for i in range(len(src_tokens)):
                    if src_tokens[i] == filter_token:
                        pos.append(i + 1)
                return pos
            ...
            (
                ...
                manipulate_positions=get_positions(
                    "梭", self.src_dict.string(src_tokens).split()
                ),
                manipulate_anchor=None,  # 2891,
                manipulate_index=11543,
                manipulate_threshold=0.25,
            )
    ...
    def hack_input(self, net_input):
        ...
        self.src_strs = [
            ...
            "他 表示 , 如果 这 次 意外 发生 的 原因 确定 是 太空 梭 老 旧 所 致 , 美国 方面 将 会 冻结 一切 太空 梭 任务 , 以免 再 发生 类似 的 灾难 事件 。",
            "如果 美国 的 太空 梭 停 摆 , 将 只 剩 俄国 能 出动 联合 号 太空船 载运 太空人 到 太空@@ 站 , 但 俄罗斯 太空 总署 目前 正 面临 严重 的 财务 拮据 问题 , 因此 美 、 俄 可能 会 达成 协议 , 利用 联合 号 维持 太空@@ 站 的 人 为 任务 , <unk> 进展 号 运 货 载 具 对 国际 太空@@ 站 进行 运 补 。",
            "这 位 俄罗斯 专家 认为 , 目前 在 国际 太空@@ 站 上 的 太空人 原 订 三月 一日 搭乘 美国 太空 梭 亚特 兰@@ 提@@ 斯 号 返回 地球 , 如今 应 会 继续 留 在 太空@@ 站 上 。",
            "俄罗斯 专家 指出 , 一日 的 不幸 事件 显示 , 太空 梭 的 不 可靠 , 俄罗斯 在 一九七一年 也 曾 出 过 意外 , 当时 联合 十一 号 太空船 在 重返 地球 时 失事 , 折 损 了 太空 船上 的 三 名 太空 人 。",
            "美@@ 西@@ 加州 和 亚@@ 利@@ 桑@@ 那 州 亦 发现 疑 似 太空 梭 残骸",
            "( 法新社 休 士 顿 电 ) 美国 国家 航空 暨 太空 总署 ( n@@ as@@ a ) 表示 , 在 美国 西部 的 加州 和 亚@@ 利@@ 桑@@ 那@@ 州 境内 亦 发现 到 疑 似 失事 哥伦比亚 号 太空 梭 的 残骸 , 此 或许 可 提供 为何 太空 梭 会 在 空中 解体 的 重要 线索 。",
            "太空 梭 计 画 经理 狄@@ 特@@ 摩 三日 表示 , 太空 总署 已 组成 一个 特别 小组 负责 在 发现 到 哥伦比亚 号 太空 梭 大部分 残骸 的 德州 和 路 易 斯@@ 安 那 州 以西 地区 搜寻 残骸 。",
            "哥伦比亚 号 太空 梭 于 一日 下降 准备 于 上午 九时 十@@ 六分 ( 台北 时间 晚间 十时 十@@ 六分 ) 降落 在 佛罗里达州 甘乃迪 太空 中心 之前 在 空中 解体 化 成 多 块 燃烧 残骸 , 太空 梭 上 的 七 名 太空人 悉数 殉职 。",
            "调查 人员 正 试图 了解 为何 在 通讯 中断 前 数 分钟 太空 梭 左@@ 侧 温度 遽 增 高 达 摄氏 十六 度 , 约 在 同 时间 太空 梭 左@@ 侧 出现 不 寻常 拉@@ 力 , 导致 太空 梭 失控 。",
        ]
        ...
        # ]
        # self.tgt_strs = [
        #     "investigators are trying to find out why there was a drastic increase of 16 degrees celsius on the left side of the space several minutes before the breakdown of the communication , and there was unusual r@@ hy@@ th@@ m on the left side of the space at about the same time , causing the space to go out of control ."
        # ]
        ...
        src_unk_num = src_string.split().count("<unk>")
        unk_test = new_src_tokens == self.src_dict.unk()
        assert unk_test.sum() - src_unk_num == 0
        ...
        if new_tgt_tokens is not None:
            return None  # new_tgt_tokens[None]
        else:
            return None
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        manipulate_positions: List[int] = None,
        manipulate_anchor: int = None,
        manipulate_index: int = None,
        manipulate_threshold: float = None,
    ):
        log_probs = []
        ...
        for i, model in enumerate(self.models):
            ...
            if manipulate_positions is not None and len(manipulate_positions) > 0:
                offset = torch.zeros(compositions_logits.size()[:2]).to(
                    compositions_logits
                )
                for manipulate_position in manipulate_positions:
                    if manipulate_anchor is None:
                        offset_pos = (
                            compositions_logits[:, :, manipulate_position].max(dim=-1)[0]
                            - compositions_logits[
                                :, :, manipulate_position, manipulate_index
                            ]
                        ...
                        offset_pos = (
                            compositions_logits[
                                :, :, manipulate_position, manipulate_anchor
                            ]
                            - compositions_logits[
                                :, :, manipulate_position, manipulate_index
                            ]
                ...
                # offset[:] = 1.0
                decoder_out_tuple[0][:, :, manipulate_index] += offset
