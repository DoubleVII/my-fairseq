...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        for step in range(step_num):
            tgt_tokens = self.tgt_strs[0].split()
            tgt_score = check_composition_logit(
                analysis_info_list, step, tgt_tokens[step]
            )[-(step + 1) :].sum()
            src_score = check_composition_logit(
                analysis_info_list, step, tgt_tokens[step]
            )[: -(step + 1)].sum()
            if sum_tgt_score is None:
                sum_tgt_score = tgt_score
                sum_src_score = src_score
            else:
                sum_tgt_score += tgt_score
                sum_src_score += src_score
        print(sum_tgt_score / step_num)
        print(sum_src_score / step_num)
        import pdb
    ...
    def hack_input(self, net_input):
        ...
        self.src_strs = [
            "深圳 罗湖 海关 严厉 打击 货币 走私",
            "德 电信 产业 mo@@ bil@@ com 创办 人 宣布 破产",  # over-trans.
            "德 电信 产业 mo@@ bil@@ e 创办 人 宣布 破产",  # hallucination
            "印尼 当局 拒绝 美国 大使馆 加强 安全 计 画",
            "哥伦比亚 汽车 炸弹 爆炸 造成 至少 4 人 死亡",
            "德 电信 产业 mo@@ bil@@ com 创办 人 施@@ 密@@ 德 宣布 破产",
            "俄 专家 : 太空 梭 灾难 可能 导致 太空@@ 站 暂停 运作",
            "澳洲 西北部 部分 地区 洪水 泛@@ 滥@@ 成灾",
        ]
        ...
        self.tgt_strs = [
            "shenzhen customs severely crack down on smuggling of money",
            "german telecom industry 's mobile phone founder announces bankruptcy",
            "bill introduced in germany for developing telecom industry .",
            "indonesian authorities reject us embassy 's safety plan",
            "bomb blast in colombia kills at least four people",
            ...
            "flooding and flooding in some parts of northwest australia",
        ]
