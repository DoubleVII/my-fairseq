...
class RelativeMultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        if not self.k_only:
            self.relative_position_values = Parameter(
                torch.Tensor(2 * self.max_relative_length + 1, self.head_dim)
            )
    ...
    def forward(
        ...
        if self.k_only:
            relation_keys = F.embedding(
                relative_positions_matrix.long().cuda(), self.relative_position_keys
            )
        else:
            relation_keys = F.embedding(
                relative_positions_matrix.long().cuda(), self.relative_position_keys
            ...
            relation_values = F.embedding(
                relative_positions_matrix.long().cuda(), self.relative_position_values
            )

        relative_attn_weights = self._relative_attention_inner(
            q, k, relation_keys, transpose=True
        )
        ...
        relative_attn_weights = F.dropout(
            relative_attn_weights, p=self.dropout, training=self.training
        )
        # key only mode
        if self.k_only:
        ...
        # original implementation
            ...
            attn = self._relative_attention_inner(
                relative_attn_weights, v, relation_values, transpose=False
            )
    ...
    def _relative_attention_inner(self, x, y, z, transpose=True):
        ...
        if transpose:
            z = z.transpose(1, 2)
        x_tz_matmul = (
            torch.bmm(x_t, z).transpose(0, 1).view(batch_size_mul_head, length, -1)
        )
