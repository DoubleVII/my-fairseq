...
def extract_features(
        model, src_tokens, token_embeddings
    ):
...
def predict(
        ...
        logits, _ = model.classification_heads[head](
            features, None
        )
        if return_logits:
            # return logits
...
def compute_grad(model, input_tokens: torch.LongTensor, pred):
        ...
        with torch.enable_grad():
            ...
            for j in range(input_tokens.size(-1)):
                sum_grads = None
                orig_embed = (
                    embed_tokens(input_tokens).clone().detach()[0, j, :]
                )
                
                alphas = torch.from_numpy(np.linspace(0, 1.0, num=num_steps, endpoint=False)).to(orig_embed)
                embed = embed_tokens(input_tokens).clone().detach() # 1 x T x C
                embed = embed.repeat((len(alphas), 1, 1)) # steps x T x C
                embed[:,j,:] *= alphas[:, None]
                embed.requires_grad_(True)
                model.zero_grad()
                lprobs = predict(model.model, "sentence_classification_head", input_tokens, embed)
