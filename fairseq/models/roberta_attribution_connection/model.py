...
@register_model("roberta_attribution_connection")
class RobertaModel(FairseqEncoderModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, extra, compositions = self.encoder(
            src_tokens,
            features_only,
            return_all_hiddens,
        ...
        if classification_head_name is not None:
            x, compositions = self.classification_heads[classification_head_name](
                x, compositions
            )
        return x, extra, compositions
...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, compositions=None, **kwargs):
        ...
        compositions = compositions[:, :, None, :]  # B x size_c x 1 x hidden_size
        x = self.dropout(x)

        compositions = forward_linear(compositions, self.dense.weight, self.dense.bias)
        # do relu
        compositions = forward_relu(compositions)
        # compositions = forward_iter_decomposition(compositions)

        compositions = forward_linear(
            compositions, self.out_proj.weight, self.out_proj.bias, iter_decompose=False
        )
        ...
        check_error(compositions, x[:, None, :])
        return x, compositions[:, :, 0, :]
...
class RobertaEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        **unused,
        ...
        """
        x, extra, compositions = self.extract_features(
            src_tokens,
    ...
    def extract_features(
        ...
        **kwargs,
        ...
        # T x B x C -> B x T x C
        compositions = encoder_out["encoder_compositions"]
        composition_states = encoder_out["composition_states"]
        connection_states = encoder_out["connection_states"]
...
@register_model_architecture("roberta_attribution_connection", "roberta_attribution_connection")
...
@register_model_architecture("roberta_attribution_connection", "roberta_prenorm_attribution_connection")
...
@register_model_architecture("roberta_attribution_connection", "roberta_base_attribution_connection")
def roberta_base_architecture(args):
    base_architecture(args)


@register_model_architecture("roberta_attribution_connection", "roberta_large_attribution_connection")
...
@register_model_architecture("roberta_attribution_connection", "xlm_attribution_connection")
